...
# The ECS Cluster
resource "aws_ecs_cluster" "reveluxlabs_ha_cluster" {
...
resource "aws_launch_configuration" "ecs_instance" {
  ...
  user_data = <<EOF
#!/bin/bash
echo "ECS_CLUSTER=${aws_ecs_cluster.reveluxlabs_ha_cluster.name}" >> /etc/ecs/ecs.config
EOF
...
resource "aws_security_group" "ecs_instance" {
  ...
  # Inbound HTTP for the reveluxlabs-site from anywhere
  ingress {
    from_port = "${var.reveluxlabs_site_port}"
    to_port = "${var.reveluxlabs_site_port}"
    protocol = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  # Inbound HTTP for the smart-machines-site from anywhere
  ingress {
    from_port = "${var.smart_machines_site_port}"
    to_port = "${var.smart_machines_site_port}"
    protocol = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  # Inbound HTTP for the sittingapp-webapp-site from anywhere
  ingress {
    from_port = "${var.sittingapp_webapp_port}"
    to_port = "${var.sittingapp_webapp_port}"
    protocol = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }
